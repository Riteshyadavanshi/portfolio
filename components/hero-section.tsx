...
import { MessageCircle, ArrowRight, MapPin, Zap, TrendingUp, BarChart3 } from "lucide-react";

const metrics = [
  { label: "Response Time", value: "-30%", icon: <Zap size={14} className="text-yellow-400" /> },
  { label: "API Performance", value: "+35%", icon: <BarChart3 size={14} className="text-blue-400" /> },
  { label: "Page Load", value: "-25%", icon: <TrendingUp size={14} className="text-green-400" /> },
];
  ...
  return (
    <div className="bento-card h-full flex flex-col justify-between group overflow-hidden bg-zinc-900/50 border-zinc-800">
      <div className="space-y-8 relative z-10">
        ...
        <div className="space-y-4">
          <motion.h1 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            className="text-4xl md:text-7xl font-bold tracking-tight leading-none text-white"
          >
          ...
          >
            Frontend-focused <span className="text-white font-bold">Full-Stack Developer</span>. 
            Engineering scalable systems at <span className="text-white font-bold">Dispatch Panda</span> with Angular & Java.
          </motion.p>
        </div>

        {/* Key Metrics Section to fill space */}
      ...
      <motion.div 
        ...
        className="flex flex-wrap gap-4 mt-12 relative z-10"
      >
        <Link href="/projects" className="group/btn">
          <button className="flex items-center gap-2 bg-white text-black px-8 py-3 rounded-full font-bold transition-all hover:bg-zinc-200 shadow-lg shadow-black/20">
            View Projects
        ...
        <Link href="/contact-me">
          <button className="flex items-center gap-2 bg-zinc-800 text-white px-8 py-3 rounded-full font-bold border border-zinc-700 hover:bg-zinc-700 transition-all shadow-sm">
